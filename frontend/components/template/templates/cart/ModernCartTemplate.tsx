...
import { CreditCard, Minus, Plus, ShoppingBag, Tag, Trash2, Truck, ArrowRight, CheckCircle } from 'lucide-react';
...
export const ModernCartTemplate: React.FC<ModernCartTemplateProps> = ({ data }) => {
  ...
  return (
    <div className={`min-h-screen bg-gray-50 transition-opacity duration-700 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
      {/* Header Section */}
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex items-center justify-between">
            <div className="space-y-2">
              ...
              <h1 className="text-3xl font-light text-gray-900 tracking-tight">Shopping Cart</h1>
            </div>
            <div className="text-right">
              <div className="text-2xl font-light text-gray-900">{items.length}</div>
              <div className="text-sm text-gray-500">{items.length === 1 ? 'item' : 'items'}</div>
            </div>
          </div>
        </div>
      </div>
              ...
              <div className="bg-white border border-gray-200 overflow-hidden">
                ...
                <div className="divide-y divide-gray-100">
                  {items.map((item, index) => (
                    <div key={item.id} className="p-6 hover:bg-gray-50 transition-all duration-200 group">
                      <div className="flex items-start space-x-6">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xl font-light text-gray-900">${item.price.toFixed(2)}</p>
                        </div>
              ...
              <div className="bg-white border border-gray-200">
                ...
                <div className="p-6">
                  {appliedPromo ? (
                    <div className="flex items-center justify-between bg-gray-50 p-4 border border-gray-200">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <p className="font-medium text-gray-900">Code Applied: {appliedPromo}</p>
                          <p className="text-gray-600 text-sm">You saved ${discount.toFixed(2)}</p>
                        </div>
                      </div>
                      <button
                        type="button"
                  ...
                  <p className="text-sm text-gray-500 mt-4">
                    Try: <code className="bg-gray-100 px-2 py-1 text-gray-700 font-mono text-xs">SAVE10</code> or{' '}
                    <code className="bg-gray-100 px-2 py-1 text-gray-700 font-mono text-xs">WELCOME20</code>
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="bg-white border border-gray-200 sticky top-8">
                ...
                <div className="p-6 space-y-4">
                  ...
                  <div className="space-y-3 mt-6">
                    ...
                    <Link href="/products">
                      <Button variant="outline" className="w-full border-gray-300 text-gray-700 hover:bg-gray-50 py-3 rounded-none transition-colors duration-200">
                        Continue Shopping
                      </Button>
                    </Link>
                  </div>
