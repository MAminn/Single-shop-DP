...
const ModernCheckoutTemplate: React.FC<ModernCheckoutTemplateProps> = ({ data }) => {
            ...
            <Card className="relative bg-white border border-gray-200 rounded-none overflow-hidden">
              <CardHeader className="bg-gray-900 text-white relative overflow-hidden">
                ...
                <div className="relative flex items-center gap-4">
                  ...
                  <div>
                    <CardTitle className="text-3xl font-light">Order Confirmed</CardTitle>
              ...
              <CardContent className="p-8">
                <div className="space-y-8">
                  ...
                  <div className="bg-gray-50 border border-gray-200 rounded-none p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="bg-white border border-gray-200 rounded-none p-4">
                        ...
                        <p className="text-lg font-light text-gray-900">{localOrderDetails.customerInfo.fullName}</p>
                      </div>
                      <div className="bg-white border border-gray-200 rounded-none p-4">
                        <p className="text-sm font-medium text-gray-500 mb-1">Phone</p>
                        <p className="text-lg font-light text-gray-900">{localOrderDetails.customerInfo.phoneNumber}</p>
                      </div>
                      <div className="bg-white border border-gray-200 rounded-none p-4 md:col-span-2">
                        <p className="text-sm font-medium text-gray-500 mb-1">Email</p>
                        <p className="text-lg font-light text-gray-900">{localOrderDetails.customerInfo.email || "Not provided"}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-50 border border-gray-200 rounded-none p-6">
                    ...
                    <div className="bg-white border border-gray-200 rounded-none p-6">
                      <div className="space-y-2">
                        <p className="text-lg font-light text-gray-900">{localOrderDetails.customerInfo.address}</p>
                        <p className="text-gray-600 font-light">
                          {localOrderDetails.customerInfo.city},{" "}
                          {localOrderDetails.customerInfo.state}{" "}
                          {localOrderDetails.customerInfo.postalCode}
                        </p>
                        <p className="text-gray-600 font-light">{localOrderDetails.customerInfo.country}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-50 border border-gray-200 rounded-none p-6">
                    ...
                    <div className="space-y-4">
                      {localOrderDetails.items.map((item) => (
                        <div key={`${item.id}-${JSON.stringify(item.selectedOptions)}`} className="bg-white border border-gray-200 rounded-none p-4">
                          <div className="flex items-center gap-4">
                              ...
                              {Object.keys(item.selectedOptions).length > 0 && (
                                <div className="flex flex-wrap gap-2 mt-2">
                                  {Object.entries(item.selectedOptions).map(([key, value]) => (
                                    <span key={key} className="bg-gray-100 text-gray-800 px-3 py-1 rounded-none text-sm font-medium border border-gray-200">
                                      {key}: {value}
                                    </span>
                                  ))}
                                </div>
                            ...
                            <div className="text-right">
                              <p className="text-2xl font-light text-gray-900">${(item.price * item.quantity).toFixed(2)}</p>
                            </div>
                          </div>
                        </div>
                      ))}
                  ...
                  {/* Order Summary */}
                  <div className="bg-gray-50 border border-gray-200 rounded-none p-6">
                    <h3 className="font-medium text-xl mb-6 text-gray-900 border-b border-gray-200 pb-3">Order Summary</h3>
                    <div className="bg-white border border-gray-200 rounded-none p-6">
                      <div className="space-y-4">
                        <div className="flex justify-between text-lg">
                          <span className="text-gray-600 font-light">Subtotal:</span>
                          <span className="font-light">${localOrderDetails.subtotal.toFixed(2)}</span>
                        </div>
                        {localOrderDetails.discount && localOrderDetails.discount > 0 && (
                          <div className="flex justify-between text-lg text-gray-900">
                            <span className="font-light">Discount:</span>
                            <span className="font-light">-${localOrderDetails.discount.toFixed(2)}</span>
                          </div>
                        )}
                        <div className="flex justify-between text-lg">
                          <span className="text-gray-600 font-light">Shipping:</span>
                          <span className="font-light">${localOrderDetails.shipping.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between text-lg">
                          <span className="text-gray-600 font-light">Tax:</span>
                          <span className="font-light">${localOrderDetails.tax.toFixed(2)}</span>
                        </div>
              ...
              <CardFooter className="p-8 bg-white border-t border-gray-200">
                <div className="flex flex-col sm:flex-row gap-4 w-full">
                  <Link href="/" className="flex-1">
                    <Button variant="outline" className="w-full h-12 text-lg font-light border border-gray-300 rounded-none hover:bg-gray-50">
  ...
  if (items.length === 0) {
    return (
      <div className={`min-h-screen bg-white transition-opacity duration-1000 ${isVisible ? 'opacity-100' : 'opacity-0'} flex items-center justify-center`}>
  ...
  return (
    <div className={`min-h-screen bg-white transition-opacity duration-1000 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
      <div className="container mx-auto p-4 max-w-7xl">
        <div className="mb-8">
          <Link href="/cart" className="inline-flex items-center gap-2 text-gray-900 hover:text-gray-600 font-light text-lg transition-colors duration-300">
              ...
              <div className="bg-gray-900 text-white relative overflow-hidden">
                ...
                <div className="relative p-6">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    ...
                    <span className="text-gray-300 font-light">SSL Encrypted & Secure</span>
                  </div>
                </div>
              </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                  ...
                  <div className="bg-gray-50 border border-gray-200 rounded-none p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      ...
                      <div>
                        <Label htmlFor="phoneNumber" className="text-sm font-medium text-gray-700 mb-2 block">Phone Number *</Label>
                        <Input
                          id="phoneNumber"
                          name="phoneNumber"
                  ...
                  <div className="bg-gray-50 border border-gray-200 rounded-none p-6">
                    ...
                    <div className="space-y-6">
                      <div>
                        <Label htmlFor="address" className="text-sm font-medium text-gray-700 mb-2 block">Street Address *</Label>
                        <Input
                          id="address"
                          name="address"
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                          <Label htmlFor="state" className="text-sm font-medium text-gray-700 mb-2 block">State/Province</Label>
                          <Input
                            id="state"
                            name="state"
                  ...
                  {/* Order Notes */}
                  <div className="bg-gray-50 border border-gray-200 rounded-none p-6">
                    <Label htmlFor="notes" className="text-sm font-medium text-gray-700 mb-2 block">Order Notes (Optional)</Label>
                    <Textarea
                      id="notes"
                      name="notes"
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="space-y-4">
          {items.map((item) => {
            ...
            return (
              <div key={`${item.id}-${optionsKey}`} className="bg-gray-50 border border-gray-200 rounded-none p-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {Object.keys(item.selectedOptions).length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-2">
                        {Object.entries(item.selectedOptions).map(([key, value]) => (
                          <span key={key} className="text-xs bg-gray-100 text-gray-800 px-2 py-1 rounded-none font-medium border border-gray-200">
                            {key}: {value}
                          </span>
                        ))}
                      </div>
