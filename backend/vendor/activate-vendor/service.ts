...
export const activateVendor = (
  input: z.infer<typeof activateVendorSchema>,
    ...
    return yield* $(
      query(async (db) => {
        return await db.transaction(async (tx) => {
          ...
          // Also verify the vendor's email if ownerId exists
          if (targetVendor.ownerId) {
            ...
            await Promise.all([activate, verifyVendorEmail, logApprove]);
          } else {
            await Promise.all([activate, logApprove]);
          }
        });
      })
    );
